...
4. Run `scripts/no-delete-guard.sh --check-diff` and `scripts/codex-auto.sh --check-budget`
   (or the same session-state logic for a non-Codex runtime) before selecting work. If the
   budget is exhausted, stop with `BUDGET_EXHAUSTED`.
...
6. Per story: ensure the integration branch exists → cut the feature branch → TDD → quality
...
7. After the story is committed and pushed successfully, increment `completedToday` and record
   `lastCompletedStory`. Never increment for failed commits, failed pushes, partial work, or
   local-only work that did not complete its configured integration step.
8. No deletes: autonomous mode may edit lines and add files, but must not delete or rename
...
9. Honor every stop condition: write `.claude/escalations.md` and halt rather than forcing
   completion or taking an unauthorized outward action.
10. Never edit specs or `discovery/` during implementation — spec changes belong to architect
   review.
11. Respect the assigned runtime roles. If `implementation_runtime` and `review_runtime`
